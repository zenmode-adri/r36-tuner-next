...
sudo(f'fdtput -t u {DTB} "{NODE}" opp-microvolt-L2 1075000')
sudo(f'fdtput -t u {DTB} "{NODE}" opp-microvolt 1075000')
sudo('rm -f /boot/.r36_dtb_pending')

v = sudo(f'fdtget -t u {DTB} "{NODE}" opp-microvolt-L2')
p = sudo('ls /boot/.r36_dtb_pending 2>/dev/null && echo EXISTS || echo gone')
print(f"DTB 600MHz: {v} uV ({int(v)/1000} mV)" if v.isdigit() else v)
print(f"DTB_PENDING: {p}")
