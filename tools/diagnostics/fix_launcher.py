...
_,out,_ = c.exec_command('echo ark | sudo -S tee /opt/system/launch_tuner.sh > /dev/null << \'EOF\'\n' + new_launcher + '\nEOF\n')
import time; time.sleep(1)

# escribirlo via stdin
stdin, stdout, stderr = c.exec_command('echo ark | sudo -S bash -c "cat > /opt/system/launch_tuner.sh"')
